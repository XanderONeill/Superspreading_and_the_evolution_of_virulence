...
%2. The function f(v) can be amended accordingly, see section (2).
...
%%% Tolerance function (see Figure 4c)
Mx = 2.0;       Mn = 0.25;      cbar = mu; %Mx>1; Mn<1; f(cbar)=1; NEW S SHAPED FUNCTION
...
parfor i = 1:n_sims
    ...
    while t < t_f
        ...
        elseif u1 < (birth +sus_death + inf_death)/R_T
            ...
            prob_vec = [0, cumsum(mu.*S)/(mu*sum(S))]*trans/R_T + (birth + sus_death + inf_death)/R_T;
            f1 = find(prob_vec < u1);
            vuln_choice = f1(end);
